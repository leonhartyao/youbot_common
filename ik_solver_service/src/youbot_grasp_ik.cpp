...
//
joint_positions_solution_t YoubotGraspIK::solvePitchRollIK(int id, double pitch, double roll,
											geometry_msgs::Point desired_position)
{
	joint_positions_solution_t exact_solution;
	// desired position as Eigen type
	Eigen::Vector3d des_position(desired_position.x, desired_position.y, desired_position.z);
	exact_solution = computeSingleIKSolution(des_position, roll, pitch, id);
	if (exact_solution.feasible)
	{
		// Set attributes of solution type
	    if (id == 1)
	    {
	    	exact_solution.arm_to_front = true;
	        exact_solution.arm_bended_up = true;
	    }
	    else if (id == 2)
	    {
	    	exact_solution.arm_to_front = true;
	        exact_solution.arm_bended_up = false;
	    }
	    else if (id == 3)
	    {
	    	exact_solution.arm_to_front = false;
	        exact_solution.arm_bended_up = true;
	    }
	    else if (id == 4)
	    {
        ...
        if (pitch >= 0)
        {
        	exact_solution.gripper_downwards = true;
        }
        else
        {
        	exact_solution.gripper_downwards = false;
        }
	}
                                                                  ...
                                                                  geometry_msgs::Vector3 desired_normal)
{
  ...
  double phi = atan2((double)des_position[1], (double)des_position[0]);
...
{
  // Need to return 1.0 if value is exactly zero!
  if (value >= 0)
    return 1.0;
  else
    return -1.0;
...
// Check whether a given configuration of the arm is feasible. i.e. within joint limits
                                                                  ...
                                                                  double des_pitch, int id)
{
  ...
  if (id == 1 || id == 2)
  {
    single_solution.joints[0] = atan2((double)des_position[1], (double)des_position[0]);
    single_solution.joints[4] = normalizeAngle(des_roll);

    double r_4 = sqrt((double)(des_position[0] * des_position[0] + des_position[1] * des_position[1])) - lox_
        - l_4_ * cos(des_pitch);
  ...
  {
    single_solution.joints[0] = normalizeAngle(atan2((double)des_position[1], (double)des_position[0]) + M_PI);
