...
int main(int argc, char **argv)
{
  ...
  ros::ServiceClient pitchtester = nh.serviceClient<trajectory_generator::CStoCS_Pitch>("From_CS_to_CS_Pitch");
  ...
  trajectory_generator::CStoCS_Pitch cs2cs_pitch;
  pnt.x = 0.3;
  pnt.y = 0.0;
  pnt.z = 0.3;
  cs2cs_pitch.request.start_pos = pnt;
  pnt.x += 0.05;
  cs2cs_pitch.request.end_pos = pnt;
  cs2cs_pitch.request.start_pitch = 0.0;
  cs2cs_pitch.request.end_pitch = 0.0;
  cs2cs_pitch.request.start_vel = 0.0;
  cs2cs_pitch.request.end_vel = 0.0;
  cs2cs_pitch.request.max_vel = 0.05;
  cs2cs_pitch.request.max_acc = 0.5;
  if (pitchtester.call(cs2cs_pitch))
  {
	  if (cs2cs_pitch.response.feasible)
	   {
		   cout << "CStoCS feasible trajectory" << endl;
		   while (!cs2cs_pitch.response.trajectory.joint_names.empty())
		   {
			   cout << cs2cs_pitch.response.trajectory.joint_names.back() << "\t";
			   cs2cs_pitch.response.trajectory.joint_names.pop_back();
		   }
		   cout << endl;
		   while (!cs2cs_pitch.response.trajectory.points.empty())
		   {
			   point = cs2cs_pitch.response.trajectory.points.back();
			   cs2cs_pitch.response.trajectory.points.pop_back();
			   cout << point.positions.back() << "\t";
			   point.positions.pop_back();
			   cout << point.positions.back() << "\t";
			   point.positions.pop_back();
	   ...
	   {
		   cout << "CS 2 CS Pitch not feasible" << endl;
	   }

  }
  else
  {
	  cout << "call service failed!" << endl;
  }
