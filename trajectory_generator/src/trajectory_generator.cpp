    ...
    DOF(5), accuracy(0), time(0), t_const(0), t_acc(0), t_dec(0)
{
  ...
  solve_roll_pitch_ik_array_client = nh.serviceClient<ik_solver_service::SolveRollPitchIKArray>("solve_roll_pitch_ik_array");
}
...
TrajectoryGenerator* TrajectoryGenerator::getTrajectory(Eigen::Vector3d & s_pos, Eigen::Vector3d & e_pos, double s_pitch,
														double e_pitch, double s_vel, double e_vel)
{
  ...
  {
    generateTrajectoriesCS(s_pos, e_pos, s_pitch, e_pitch, s_vel, e_vel);
    transformTrajectoryCStoJSPitch();
 ...
 * All times are discrete with time step according to the loop rate.
 */
int TrajectoryGenerator::calculateMaxVelsCS(Eigen::Vector3d& p_start, Eigen::Vector3d& p_end, double start_pitch, double end_pitch,
		  	  	  	  	 double start_v, double end_v)
{
  //cout << "Calculating Max Vel" << endl;
  double T;
  double pitch_diff;
  Eigen::Vector3d q_diff, act_diff, old_vel, act_vel, tar_vel, q_norm, temp;

  q_diff = p_end - p_start;
  ...
  if (pitch_diff > 0.01)
  {
	  pitch_v = pitch_diff / (t_acc + t_const + t_dec);
  }
  else
  {
	  pitch_v = 0.0;
  }
  cout << "pitch_diff: \t" << pitch_diff << endl;
  cout << "pitch_v: \t" << pitch_v << endl;
 ...
 */
int TrajectoryGenerator::generateTrajectoriesCS(Eigen::Vector3d& p_start, Eigen::Vector3d& p_end, double start_pitch,
												double end_pitch, double start_v, double end_v)
{
  ...
  cout << p_end.matrix();
  cout << endl;
  calculateMaxVelsCS(p_start, p_end, start_pitch, end_pitch, start_v, end_v);
  //cout << "Generating Trajectory" << endl;
  double T;
  double cur_pitch = start_pitch;
  Eigen::Vector3d cur_pos, cur_vel, cur_acc, q_diff, q_norm;
  q_diff = p_end - p_start;
  q_norm = q_diff;
  q_norm.normalize();
  ...
  pitch_tra.insert(pitch_tra.begin(), cur_pitch);
  T = t_step;
  while (fabs(T - t_acc - t_step) > accuracy / 1000.0)
  {
    ...
    cur_pitch = pitch_tra.front() + pitch_v;
    pos_tra.insert(pos_tra.begin(), cur_pos);
    vel_tra.insert(vel_tra.begin(), cur_vel);
    acc_tra.insert(acc_tra.begin(), cur_acc);
    pitch_tra.insert(pitch_tra.begin(), cur_pitch);
 ...
 */
int TrajectoryGenerator::transformTrajectoryCStoJSPitch()
{
  cout << "Transforming CS to JS Trajectory, this might take a while..." << endl;
  Eigen::VectorXd cur_pos(5), next_pos(5), prev_pos(5), cur_vel(5), next_vel(5), cur_acc(5);
  Eigen::Vector3d tra_pos;
  double tra_pitch;
  Eigen::VectorXd joint_vel(5);
  double roll, pitch, yaw;
  ik_solver_service::SolveRollPitchIKArray rp_srv;
  ik_solver_service::RollPitchReq request;
  ik_solver_service::FullyConstrainedRes response;
  std::vector<double>::iterator itpitch = pitch_tra.end() - 1;
  for (std::vector<Eigen::Vector3d>::iterator it = pos_tra.end() - 1; it != pos_tra.begin() - 1; it--)
  {
    ...
    request.id = 1;
    request.roll = 0.0;
    request.pitch = tra_pitch;
    request.des_position[0] = tra_pos[0];
    request.des_position[1] = tra_pos[1];
    request.des_position[2] = tra_pos[2];
    rp_srv.request.ikarray.insert(rp_srv.request.ikarray.begin(), request);
    itpitch--;
  }
  if (solve_roll_pitch_ik_array_client.call(rp_srv))
  {
    while (!rp_srv.response.ikresp.empty())
    {
      response = rp_srv.response.ikresp.back();
      rp_srv.response.ikresp.pop_back();
      ...
      {
        cur_pos.setZero(DOF);
        cout << "Roll Pitch IK not feasible" << endl;
        q_tra.clear();
        return -1;
      }
      q_tra.insert(q_tra.begin(), cur_pos);
                                                    ...
                                                    Eigen::Vector3d center)
{
  ...
  else
  {
    ...
    // make eef reach target plain orthogonal
    for(int i=0; i<50; i++)
    {
        cur_pos(0) -= 0.0002;
        pos_tra.insert(pos_tra.begin(), cur_pos);
        rpy_tra.insert(rpy_tra.begin(), cur_rpy);
        pos_tra.insert(pos_tra.end(), cur_pos);
        rpy_tra.insert(rpy_tra.end(), cur_rpy);
    }
