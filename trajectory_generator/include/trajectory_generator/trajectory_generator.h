...
public:
  ...
  TrajectoryGenerator* getTrajectory(Eigen::Vector3d & s_pos, Eigen::Vector3d & e_pos, double s_pitch,
...
private:
  ...
  int calculateMaxVelsCS(Eigen::Vector3d& p_start, Eigen::Vector3d& p_end, double start_pitch, double end_pitch,
  ...
  int generateTrajectoriesCS(Eigen::Vector3d& p_start, Eigen::Vector3d& p_end, double start_pitch, double end_pitch,
		  	  	  	  	  	 double start_v, double end_v);
  ...
  int transformTrajectoryCStoJSPitch();
  int generateTorques();
  ...
  //define variables
  int DOF;
  double max_vel, max_acc, max_dec, t_step, accuracy, time, t_const, t_acc, t_dec, pitch_v;
